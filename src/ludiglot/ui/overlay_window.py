...
def _check_and_clone_wuthering_data(config_path: Path) -> bool:
    """检测WutheringData是否存在，如不存在则询问用户是否克隆。
    ...
    data_root_str = config_raw.get("data_root")
    
    # 如果没有配置data_root或已经存在，直接返回
    ...
    # 询问用户是否克隆WutheringData
    msg = QMessageBox()
    msg.setIcon(QMessageBox.Icon.Question)
    msg.setWindowTitle("WutheringData 未找到")
    msg.setText(f"未找到 WutheringData 数据目录：\n{data_root}")
    msg.setInformativeText(
        "WutheringData 是鸣潮游戏的文本和音频数据库。\n\n"
        "是否自动从 GitHub 克隆？\n"
        "(仓库大小约 200MB，需要 git 命令)"
    )
    ...
    if yes_btn:
        yes_btn.setText("克隆 (Clone)")
    if no_btn:
        no_btn.setText("跳过 (Skip)")
    if cancel_btn:
        cancel_btn.setText("取消 (Cancel)")
    ...
    try:
        ...
        progress_msg.setWindowTitle("正在克隆")
        progress_msg.setText("正在从 GitHub 克隆 WutheringData...")
        progress_msg.setInformativeText("这可能需要几分钟，请耐心等待。")
        ...
        result = subprocess.run(
            ["git", "clone", "https://github.com/Dimbreath/WutheringData.git", str(data_root)],
            capture_output=True,
            text=True,
            timeout=600  # 10分钟超时
        )
        ...
        if result.returncode != 0:
            ...
            error_msg.setWindowTitle("克隆失败")
            error_msg.setText("克隆 WutheringData 失败")
        ...
        success_msg.setWindowTitle("克隆成功")
        success_msg.setText("WutheringData 已成功克隆！")
        success_msg.setInformativeText(f"位置：{data_root}")
        success_msg.exec()
        return True
        ...
        error_msg = QMessageBox()
        error_msg.setIcon(QMessageBox.Icon.Critical)
        error_msg.setWindowTitle("克隆超时")
        error_msg.setText("克隆操作超时（超过10分钟）")
        error_msg.setInformativeText("请检查网络连接或手动克隆仓库。")
        error_msg.exec()
        return False
    except FileNotFoundError:
        error_msg = QMessageBox()
        error_msg.setIcon(QMessageBox.Icon.Critical)
        error_msg.setWindowTitle("Git 未安装")
        error_msg.setText("未找到 git 命令")
        error_msg.setInformativeText(
            "请先安装 Git：\n"
        ...
        error_msg = QMessageBox()
        error_msg.setIcon(QMessageBox.Icon.Critical)
        error_msg.setWindowTitle("克隆出错")
        error_msg.setText("克隆过程中出现错误")
...
def run_gui(config_path: Path) -> None:
    ...
    # 在加载配置前先检查WutheringData
    if not _check_and_clone_wuthering_data(config_path):
        return  # 用户取消操作，退出
