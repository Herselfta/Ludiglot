...
Ludiglot 目前仅支持以下原生本地 OCR 后端：

1. **Windows OCR (唯一支持后端)** → 速度极快（<0.05s）、完全本地运行，内存占用低（约50MB），不需要复杂的深度学习环境。
...
- **语言包**：需要在 Windows 设置中安装对应语言的 OCR 功能包。
...
```json
{
  ...
  "ocr_backend": "auto"       // 后端选择：auto/windows
}
```
